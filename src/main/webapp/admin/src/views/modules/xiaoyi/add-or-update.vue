    ...
    >
      ...
      <el-col :span="12">
        ...
        <div v-else>
          ...
          <el-row>
            <el-col :span="24">
              <el-form-item v-if="type!='info'"  label="校医简介" prop="xiaoyijianjie">
                <editor 
                    style="min-width: 200px; max-width: 600px;"
                    v-model="ruleForm.xiaoyijianjie" 
                    class="editor" 
                    action="file/upload">
                </editor>
              </el-form-item>
              <div v-else>
                <el-form-item v-if="ruleForm.xiaoyijianjie" label="校医简介" prop="xiaoyijianjie">
                    <span v-html="ruleForm.xiaoyijianjie"></span>
                </el-form-item>
              </div>
            </el-col>
          </el-row>
          <el-row>
            <el-col :span="24">
              <el-form-item v-if="type!='info'"  label="排班表" prop="paibanbiao">
                <editor 
                    style="min-width: 200px; max-width: 600px;"
                    v-model="ruleForm.paibanbiao" 
                    class="editor" 
                    action="file/upload">
                </editor>
              </el-form-item>
              <div v-else>
                <el-form-item v-if="ruleForm.paibanbiao" label="排班表" prop="paibanbiao">
                    <span v-html="ruleForm.paibanbiao"></span>
                </el-form-item>
              </div>
            </el-col>
          </el-row>
...
export default {
  data() {
	...
	clicktime : false,
      ...
      rules: {
          xiaoyigonghao: [
                { required: true, message: '校医工号不能为空', trigger: 'blur' },
          ],
          xiaoyixingming: [
                { required: true, message: '校医姓名不能为空', trigger: 'blur' },
          ],
          keshi: [
          ],
          mima: [
  ...
  methods: {
    ...
    init(id,type) {
      ...
      if(this.type=='info'||this.type=='else'){
	    ...
	    this.ro.zuozhenshijian = true;
          ...
          if(o=='xiaoyijianjie'){
            this.ruleForm.xiaoyijianjie = obj[o];
	    this.ro.xiaoyijianjie = true;
            continue;
          }
          if(o=='paibanbiao'){
            this.ruleForm.paibanbiao = obj[o];
	    this.ro.paibanbiao = true;
            continue;
          }
          if(o=='clicktime'){
	...
	this.ruleForm.xiaoyijianjie = this.ruleForm.xiaoyijianjie.replace(reg,'../../../ssmc7l26/upload');
	this.ruleForm.paibanbiao = this.ruleForm.paibanbiao.replace(reg,'../../../ssmc7l26/upload');
        } else {
          this.$message.error(data.msg);
        }
      });
		 ...
		 if(crossrefid && crossuserid) {
			...
			this.$http({ 
				...
				if (data && data.code === 0) { 
					 ...
					 this.$http({
					   url: `xiaoyi/${!this.ruleForm.id ? "save" : "update"}`,
					   method: "post",
					   data: this.ruleForm
					 }).then(({ data }) => {
					   if (data && data.code === 0) {
						 ...
						 this.parent.xiaoyiCrossAddOrUpdateFlag = false;
						 this.parent.search();
