<template>
  <div class="main-content">
    ...
    <div v-if="showFlag">
      <el-form :inline="true" :model="searchForm" class="form-content">
        <el-row  :gutter="20" class="slt" :style="{justifyContent:contents.searchBoxPosition=='1'?'flex-start':contents.searchBoxPosition=='2'?'center':'flex-end'}">
                ...
                <el-form-item :label="contents.inputTitle == 1 ? '药品正式名' : ''">
                  <el-input v-if="contents.inputIcon == 1 && contents.inputIconPosition == 1" prefix-icon="el-icon-search" v-model="searchForm.yaopinzhengshiming" placeholder="药品正式名" clearable></el-input>
                  <el-input v-if="contents.inputIcon == 1 && contents.inputIconPosition == 2" suffix-icon="el-icon-search" v-model="searchForm.yaopinzhengshiming" placeholder="药品正式名" clearable></el-input>
                  <el-input v-if="contents.inputIcon == 0" v-model="searchForm.yaopinzhengshiming" placeholder="药品正式名" clearable></el-input>
                </el-form-item>
          <el-form-item>
        ...
        <el-row class="ad" :style="{justifyContent:contents.btnAdAllBoxPosition=='1'?'flex-start':contents.btnAdAllBoxPosition=='2'?'center':'flex-end'}">
          <el-form-item>
            ...
            <el-button
              v-if="isAuth('yaopinxinxi','报表') && contents.btnAdAllIcon == 1 && contents.btnAdAllIconPosition == 1"
              type="warning"
              icon="el-icon-s-data"
              @click="chartDialog()"
            >{{ contents.btnAdAllFont == 1?'统计报表':'' }}</el-button>
            <el-button
              v-if="isAuth('yaopinxinxi','报表') && contents.btnAdAllIcon == 1 && contents.btnAdAllIconPosition == 2"
              type="warning"
              @click="chartDialog()"
            >{{ contents.btnAdAllFont == 1?'统计报表':'' }}<i class="el-icon-s-data el-icon--right" /></el-button>
            <el-button
              v-if="isAuth('yaopinxinxi','报表') && contents.btnAdAllIcon == 0"
              type="warning"
              @click="chartDialog()"
            >{{ contents.btnAdAllFont == 1?'统计报表':'' }}</el-button>
        ...
        <el-table class="tables" :size="contents.tableSize" :show-header="contents.tableShowHeader"
		    ...
		    label="药品名称">
		     <template slot-scope="scope">
                ...
                <el-table-column  :sortable="contents.tableSortable" :align="contents.tableAlign" 
                    prop="yaopinzhengshiming"
                   :header-align="contents.tableAlign"
		    label="药品正式名">
		     <template slot-scope="scope">
                       {{scope.row.yaopinzhengshiming}}
                     </template>
                </el-table-column>
            ...
            <el-table-column width="300" :align="contents.tableAlign" 
               :header-align="contents.tableAlign"
                ...
                <el-button v-if="isAuth('yaopinxinxi','立即购买') && contents.tableBtnIcon == 1 && contents.tableBtnIconPosition == 1" type="success" icon="el-icon-tickets" size="mini" @click="yaopindingdanCrossAddOrUpdateHandler(scope.row,'cross','','','')">{{ contents.tableBtnFont == 1?'立即购买':'' }}</el-button>
                <el-button v-if="isAuth('yaopinxinxi','立即购买') && contents.tableBtnIcon == 1 && contents.tableBtnIconPosition == 2" type="success" size="mini" @click="yaopindingdanCrossAddOrUpdateHandler(scope.row,'cross','','','')">{{ contents.tableBtnFont == 1?'立即购买':'' }}<i class="el-icon-tickets el-icon--right" /></el-button>
                <el-button v-if="isAuth('yaopinxinxi','立即购买') && contents.tableBtnIcon == 0" type="success" size="mini" @click="yaopindingdanCrossAddOrUpdateHandler(scope.row,'cross','','','')">{{ contents.tableBtnFont == 1?'立即购买':'' }}</el-button>
    ...
    <yaopindingdan-cross-add-or-update v-if="yaopindingdanCrossAddOrUpdateFlag" :parent="this" ref="yaopindingdanCrossaddOrUpdate"></yaopindingdan-cross-add-or-update>
    ...
    <el-dialog
      ...
      width="800">
        <div id="yaopinshuliangChart" style="width:100%;height:600px;"></div>
...
import yaopindingdanCrossAddOrUpdate from "../yaopindingdan/add-or-update";
export default {
  data() {
    return {
      searchForm: {
      ...
      yaopindingdanCrossAddOrUpdateFlag: false,
  ...
  methods: {
    ...
    yaopindingdanCrossAddOrUpdateHandler(row,type,crossOptAudit,statusColumnName,tips,statusColumnValue){
      this.showFlag = false;
      this.addOrUpdateFlag = false;
      this.yaopindingdanCrossAddOrUpdateFlag = true;
		...
		for (var o in obj){
		  if(o==statusColumnName && obj[o]==statusColumnValue){
			...
			this.getDataList();
		      ...
		      this.showFlag = true;
		      this.yaopindingdanCrossAddOrUpdateFlag = false;
			return;
		  }
		}
	}
      this.$nextTick(() => {
      this.$refs.yaopindingdanCrossaddOrUpdate.init(row.id,type);
      });
    },
    chartDialog() {
      ...
      this.$nextTick(()=>{
        ...
        //  yaopinshuliang
        // xcolumn ycolumn yaopinmingcheng yaopinshuliang
        var yaopinshuliangChart = this.$echarts.init(document.getElementById("yaopinshuliangChart"),'macarons');
        this.$http({
            url: `yaopinxinxi/value/yaopinmingcheng/yaopinshuliang`,
            method: "get"
        }).then(({ data }) => {
            if (data && data.code === 0) {
                let res = data.data;
                ...
                for(let i=0;i<res.length;i++){
                    ...
                    // 使用刚指定的配置项和数据显示图表。
                    yaopinshuliangChart.setOption(option);
                    //根据窗口的大小变动图表
                    window.onresize = function() {
                        yaopinshuliangChart.resize();
                    };
                }
            }
        });
    ...
    getDataList() {
      ...
      let params = {
        ...
        sort: 'id',
          ...
          if(this.searchForm.yaopinzhengshiming!='' && this.searchForm.yaopinzhengshiming!=undefined){
            params['yaopinzhengshiming'] = '%' + this.searchForm.yaopinzhengshiming + '%'
          }
      this.$http({
